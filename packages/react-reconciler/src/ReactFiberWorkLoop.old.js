...
// 1. 找到优先级最高的Fiber Root
// 2. 判断root === workInProgressRoot
// 3. 获得当前更新的优先级
// 4. lane === SyncLane 则说明是同步任务，调用performSyncWorkOnRoot立即执行
// 5. 不是同步任务，调用ensureRootIsScheduled
export function scheduleUpdateOnFiber(
  ...
  if (lane === SyncLane) {
    if (
      // 处于unbatchedUpdates， 且不在Renderer渲染阶段， 立即执行
...
// // exiting a task.
// 使用此函数为根目录安排任务。 每个根只有一个任务。
// 如果已经安排了任务，我们将进行检查以确保现有任务的优先级与根已处理的下一级别的优先级相同。 
// 每次更新时都会在退出任务之前调用此函数。
function ensureRootIsScheduled(root: FiberRoot, currentTime: number) {
  // root.callbackNode的存活周期是从ensureRootIsScheduled开始——>到commitRootImpl截止
  ...
  if (existingCallbackNode !== null) {
    ...
    // 优先级没有改变。 我们可以重用现有任务, 现有任务的优先级和下一个任务的优先级相同。比如input连续的输入，优先级相同，可以执行用之前的任务
    // 由于获取更新是从root开始，往下找到在这个优先级内的所有update.
    // 比如存在连续的setState，会执行这个逻辑，不会新建一个新的update
    if (existingCallbackPriority === newCallbackPriority) {
      // The priority hasn't changed. We can reuse the existing task. Exit.
      return;
    }
    // 优先级变了，先cancel掉，后续重新发起一个，// 中断逻辑吗？
...
// 1. 向上递归completedWork
// 2. 创建DOM节点，更新Dom节点
// 3. 把子节点的side Effect统统附加到父节点的sideEffect链之上
